...
Los scripts crean/reutilizan un entorno virtual `venv`, instalan las dependencias definidas en `requirements.txt` y lanzan el menú interactivo del CLI. Si existe una sesión guardada válida (`session.json` + `session_meta.json`), el programa la restaurará automáticamente al iniciar para que puedas ir directo a las opciones de scraping.
...
> 💡 Si necesitas instalar manualmente en Python 3.14+, ejecuta primero `pip install --pre --only-binary=:all: pydantic-core>=2.27.0 pydantic>=2.9.2` y luego `pip install -r requirements-base.txt`.
